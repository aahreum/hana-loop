...
- [x] **DateRangePicker (Popover)** — 트리거 버튼(현재 범위 표시) + Popover 안에 From/To 직접 선택 + Preset 4개(`전체` / `최근 1개월` / `3개월` / `6개월`)
  ...
  - From/To 월 input 은 키보드 직접 입력 및 붙여넣기 차단(`onKeyDown` 에서 필수 키 외 preventDefault, `onPaste` 차단) — native month input 이 한국 로케일에서 5자리 이상 연도(`123444년`) 같은 비현실적 값을 받아들이는 문제 회피. picker UI 로만 선택. 접근성을 위해 Tab/Enter/Space/Escape 는 허용 (포커스 이동 / picker 열기·닫기)
